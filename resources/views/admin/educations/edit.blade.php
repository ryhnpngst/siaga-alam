<x-admin.layout>
    ...
    <section class="bg-white dark:bg-gray-900 p-3 sm:p-5 shadow-md sm:rounded-lg">
        <div class="py-8 mx-auto max-w-2xl ">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit informasi</h2>
            <form action="{{ route('educations.update', $education->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    @enderror"
                            placeholder="Ketik judul artikel" value="{{ old('title', $education->title) }}">
                        @error('title')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="body" name="body" rows="8"
                            ...
                            placeholder="Ketik isi artikel">{{ old('body', $education->body) }}</textarea>
                        @error('body')
                            <p class="mt-2 peer-invalid:visible text-pink-600 text-sm">
                                {{ $message }}
                            </p>
