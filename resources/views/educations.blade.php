                ...
                @forelse ($educations as $education)
                    <article
                        ...
                        <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white hover:underline">
                            <a href="/educations/{{ $education['slug'] }}">{{ $education['title'] }}</a>
                        </h2>
                        <p class="mb-5 font-light text-gray-500 dark:text-gray-400">
                            {{ Str::limit(strip_tags($education['body']), 100) }}</p>
                        <div class="flex justify-between items-center">
                            <div class="flex items-center space-x-4">
                                <div
                                    class="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600">
                                    <span
                                        class="font-medium text-gray-600 dark:text-gray-300">{{ $education->author->initials }}</span>
                                </div>
                                <span class="font-medium dark:text-white hover:underline">
                                    <a href="/educations?author={{ $education->author->username }}">
                                        {{ $education->author->name }}
                                    </a>
                                </span>
                            </div>
                            <a href="/educations/{{ $education['slug'] }}"
                        ...
                        <figcaption class="flex items-center justify-center mt-6 space-x-3">
                            <div class="flex items-center divide-x-2 divide-gray-500 dark:divide-gray-700">
                                ...
                                <div class="pl-3 text-sm font-light text-blue-600">
                                    <a href="/educations" class="flex items-center hover:underline">
